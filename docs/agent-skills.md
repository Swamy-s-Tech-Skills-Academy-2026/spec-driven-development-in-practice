...
Bundled procedures live under `.github/skills/` as `SKILL.md` files (YAML frontmatter + Markdown body).
The same tree is mirrored at `.cursor/skills/`; parity is checked in CI when those paths change.

Markdown lint for this repository uses **`.markdownlint-cli2.yaml`** at the repo root (including a `frontMatter`
pattern so skill files lint correctly).
...
| ------------------- | ----------------------------------------------------------- |
| `sdd-learning`      | Editing topic Markdown — scope, voice, zero-copy, SDD map |
